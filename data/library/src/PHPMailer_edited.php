...
class PHPMailer
{
    ...
    protected function mailSend($header, $body)
    {
        ...
        foreach ($this->to as $toaddr) {
            array_push($toArr, $this->addrFormat($toaddr));
        }
        $to = implode(', ', $toArr);
    ...
    public function addrAppend($type, $addr)
    {
        ...
        foreach ($addr as $address) {
            array_push($addresses, $this->addrFormat($address));
        }
    ...
    public function createHeader()
    {
        ...
        if ($this->SingleTo) {
            if ('mail' != $this->Mailer) {
                foreach ($this->to as $toaddr) {
                    array_push($this->SingleToArray, $this->addrFormat($toaddr));
                }
            }
        } else {
            if (count($this->to) > 0) {
    ...
    protected function setMessageType()
    {
        ...
        if ($this->inlineImageExists()) {
            array_push($type, 'inline');
        }
        if ($this->attachmentExists()) {
            array_push($type, 'attach');
        }
    ...
    protected function attachAll($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] == $disposition_type) {
                ...
                array_push($mime, sprintf('--%s%s', $boundary, static::$LE));
                //Only include a filename property if we have one
                if (!empty($name)) {
                    array_push($mime, sprintf(
                ...
                // RFC1341 part 5 says 7bit is assumed if not specified
                if ('7bit' != $encoding) {
                    array_push($mime, sprintf('Content-Transfer-Encoding: %s%s', $encoding, static::$LE));
                }

                if (!empty($cid)) {
                    array_push($mime, sprintf('Content-ID: <%s>%s', $cid, static::$LE));
                }
                ...
                if (!(empty($disposition))) {
                    ...
                    array_push($mime, static::$LE);
                }

                // Encode as string attachment
                if ($bString) {
                    array_push($mime, $this->encodeString($string, $encoding));
                } else {
                    array_push($mime, $this->encodeFile($path, $encoding));
                }
                if ($this->isError()) {
                    return '';
                }
    ...
    public function addStringAttachment(
        ...
        array_push($this->attachment, array(
            0 => $string,
            1 => $filename,
            2 => basename($filename),
            3 => $encoding,
    ...
    public function addCustomHeader($name, $value = null)
    {
        if (null === $value) {
            ...
            array_push($this->CustomHeader, explode(':', $name, 2));
        } else {
            array_push($this->CustomHeader, array($name, $value));
        }
    }
